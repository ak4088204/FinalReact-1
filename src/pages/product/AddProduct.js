...
const ProductTable = () => {
 ...
 React.useEffect(() => {
  ...
  if(bar!=="")
  axios.get(`http://localhost:8080/api/pg?barcode=${bar}`).then(res=>setids(res.data)).catch(err=>console.log(err)); 
...
React.useEffect(() => {
  ...
  axios.post("http://localhost:8080/api/inventory",j).then(
      (response2)=>{
      ...
      const verdict={
       ...
       axios.post("http://localhost:8080/api/purchase-orderss",verdict).then(res=>console.log(res)).then(err=>console.log(err));
...
const handleAddProduct = () => {
  ...
  axios.post("http://localhost:8080/api/products", json).then(res=>{
    setbar(barcode);
  setquan(quantity);
  setur(location);
  ...
  axios.post("http://localhost:8080/api/barcode-scanners",bars).then(res=>console.log(res)).catch(err=>console.log(err));
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div id="model" class="modal-body">
       ...
       <b>Supplier:</b><input id="supp"  ></input><br></br><br></br>
